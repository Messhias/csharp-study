...
public class GameState
{
        ...
        Values valueToAskFor, Deck stock)
    {
        ...
        var enumerateCards = cards as Card[] ?? cards.ToArray();
        if (enumerateCards.Length != 0)
        {
            player.AddCardsAndPullOutBooks(enumerateCards);
            message += $"{playerToAsk.Name} has {enumerateCards.Length}"
                       + $" {valueToAskFor} card{Player.S(enumerateCards.Length)}";
        }
